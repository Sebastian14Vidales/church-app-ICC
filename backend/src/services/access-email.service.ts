...
export const sendConfirmationEmail = async ({ email, name, token }: AccessEmailPayload) => {
  ...
  await sendAccessEmail({
    ...
    intro: "Abre el enlace para activar tu cuenta y definir tu contrasena final.",
...
export const sendPasswordResetEmail = async ({ email, name, token }: AccessEmailPayload) => {
  ...
  await sendAccessEmail({
    ...
    name,
    token,
    subject: `Restablece tu contrasena en ${MAIL_APP_NAME}`,
    eyebrow: "Recuperacion de acceso",
    heading: "recibimos una solicitud para cambiar tu contrasena.",
    intro: "Abre el enlace seguro para crear una nueva contrasena de acceso.",
